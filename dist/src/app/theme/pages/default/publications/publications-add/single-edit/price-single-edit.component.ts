...
@Component({
    selector: 'app-price-single-edit',
    templateUrl: './price-single-edit.component.html',
...
export class PriceSingleEditComponent implements OnInit, AfterViewInit {
    ...
    constructor(public formBuilder: FormBuilder, private _countryService: CountryService,  private _salutationService: SalutationService, private _languageService: LanguageService, private _publicationService:PublicationService, private activeRoute: ActivatedRoute, private _router:Router) {
        this.formInit();
    ...
    formInitChildPriceGrid(){
        setTimeout(() => {
        $('.m_select_available_country,.m_select_currency').selectpicker('refresh');
        }, 0);
    }
    ...
    getFormEdit(){
        ...
        return this._publicationService.PublicationSingleEdit(this.routeParams.singleEdit).subscribe(
            data => {
    ...
    editinitPriceListFiled(PriceList): FormGroup{
        ...
        Format : [PriceList.Format, Validators.required],
        Price : [PriceList.Price, Validators.required],
   ...
   initPriceListFiled(): FormGroup{
    ...
    create() {
        ...
        if (!this.createForm.valid) {
            ...
            Helpers.setLoading(true);
            this._publicationService.publicationSingleUpdate(this.createForm.value,this.routeParams.singleEdit).subscribe(
                data => {
